...
const FieldManager = (() => {
    ...
    function detectConditionals() {
        ...
        for (let i = 0; i < _fields.length; i++) {
            ...
            if (!(optLower.includes('si') && optLower.includes('no'))) continue;

            // Look ahead for dependent fields (check obs for conditional hints)
            const dependents = [];
            for (let j = i + 1; j < _fields.length; j++) {
                const next = _fields[j];
                // Stop if we hit a new section or another Si/No question
                if (next.step !== field.step) break;

                const nextObs = (next.obs || '').toLowerCase();
                const fieldNameLower = field.fieldName.toLowerCase();

                // Check if this field depends on the Si/No answer
                const isDependent = nextObs.includes('en caso de si') ||
                ...
                if (isDependent && nextObs.includes('si')) {
