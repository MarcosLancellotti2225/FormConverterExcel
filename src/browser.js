...
const { generateSignframeJson } = require('./signframe-generator/index');
...
async function runSignframeGenerator(inputs) {
    ...
    if (!pdfFile) throw new Error('Cargá el PDF');
...
// ETAPA 1: parse matrix + signframe JSON, return every JSON field with its
// best matrix suggestion plus the matrix rows for the picker.
async function runSignframePrepare(inputs) {
    const { prepareMapping } = require('./signframe-generator/mapper');
    const { matrixFile, signframeJsonFile } = inputs;
...
// CANONICAL MATRIX GENERATOR — collapse the renamed-PDF mapping xlsx into a
// canonical 2-sheet business matrix (Campos + Opciones). Optionally merges
// business rules from the Ficha original.
async function runCanonicalMatrix(inputs) {
    const { buildCanonicalMatrix } = require('./signframe-generator/canonical-matrix');
...
async function runSignframeGenerateFromMapping(inputs) {
    ...
    const { matrixFile, signframeJsonFile, targetJsonFile, mapping } = inputs;
    if (!signframeJsonFile) throw new Error('Cargá el JSON de Signframe');
    if (!matrixFile) throw new Error('Cargá la matriz XLSX');
    if (!mapping) throw new Error('Falta el vínculo (Etapa 1)');
    const matrixBytes = await fileToUint8Array(matrixFile);
    const signframeJson = JSON.parse(await fileToText(signframeJsonFile));
    const targetJsonText = targetJsonFile ? await fileToText(targetJsonFile) : null;
...
async function mergePdfs(pdfFiles) {
    ...
    // Fix /P references and rebuild AcroForm
    ...
    for (let pi = 0; pi < pages.length; pi++) {
        ...
        for (let ai = 0; ai < annots.size(); ai++) {
            ...
            // Fix /P to point to the actual page in the merged doc
            dict.set(PDFName.of('P'), pageRef);

            // Collect top-level field ref for AcroForm Fields array
...
// ─── PDF metadata (Document Info dictionary) ──────────────────────────────────
...
async function readPdfMetadata(input) {
    ...
    const iso = function (v) { try { return v ? v.toISOString() : ''; } catch (e) { return ''; } };
    return {
        title: doc.getTitle() || '',
...
async function runJsonCompare(inputs) {
    ...
    if (!analysis) throw new Error('Primero cargá el form-definition');
    if (!sampleFile) throw new Error('Cargá el JSON de ejemplo del cliente');
    const text = await fileToText(sampleFile);
    let sample;
    try { sample = JSON.parse(text); }
    catch (e) { throw new Error('El ejemplo no es un JSON válido: ' + e.message); }
    return compareWithSample(analysis, sample, direction || 'output');
}

// Reporte comercial (HTML de una página) a partir de una cotización.
function buildQuoteReport(result, opts) {
    const { buildCommercialReport } = require('./quoter/report');
    return buildCommercialReport(result, opts || {});
}

// El mismo reporte en Word (.docx real, generado con OOXML).
function buildQuoteDocx(result, opts) {
    const { buildDocxReport } = require('./quoter/docx');
    return buildDocxReport(result, opts || {});
}
...
async function runQuoteZip(inputs) {
    const { analyzeZip } = require('./quoter/index');
    const { zipFile, config } = inputs;
    if (!zipFile) throw new Error('Cargá un archivo ZIP');
...
async function writePdfMetadata(input, meta) {
    ...
    if (meta.title !== undefined) doc.setTitle(String(meta.title || ''));
    if (meta.author !== undefined) doc.setAuthor(String(meta.author || ''));
    if (meta.subject !== undefined) doc.setSubject(String(meta.subject || ''));
    if (meta.keywords !== undefined) {
        const kw = String(meta.keywords || '').split(',').map(function (k) { return k.trim(); }).filter(Boolean);
        doc.setKeywords(kw);
    }
    if (meta.creator !== undefined) doc.setCreator(String(meta.creator || ''));
    if (meta.producer !== undefined) doc.setProducer(String(meta.producer || ''));
    if (meta.creationDate) { const dt = new Date(meta.creationDate); if (!isNaN(dt.getTime())) doc.setCreationDate(dt); }
    if (meta.modificationDate) { const dt = new Date(meta.modificationDate); if (!isNaN(dt.getTime())) doc.setModificationDate(dt); }
